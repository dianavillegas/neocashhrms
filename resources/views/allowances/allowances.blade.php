   ...
   <div class="br-pageheader pd-y-15 pd-l-20">
      ...
      <div class="d-sm-flex align-items-center pd-t-15 pd-sm-t-25 pd-x-20 pd-sm-x-30">
        ...
        <div class="pd-sm-l-20 mg-t-10 mg-sm-t-0">
          <h4 class="tx-gray-800 mg-b-5">List of Allowances</h4>
        </div>
       <div class="col-md-8"></div>
        
        <div class="col-md-1">
                <a class="btn btn-primary pull-right" href="{{route('createallowancetype')}}">Add New Allowance Type</a>
              </div>
      </div><!-- d-flex -->
        ...
        <div class="br-section-wrapper">
          ...
          <div class="table-wrapper">
            <table id="datatable1" class="table display responsive nowrap">
              ...
              <tbody>
                @foreach($allowancetypes as $al)
                <tr>
                    <td>{{$al->id}}</td>
                    <td>{{$al->name}}</td>
                    <td><button type="submit" class="btn btn-success" onclick="getal('{{$al->id}}')">Edit</button></td>
                </tr>
              @endforeach
              </tbody>
            </table>
    ...
    function getal(id){
      window.location.href = '{{route ("editallowancetype", ["id" => ''] )}}'+'/'+id
    }
